...
public class P_Movement : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Space) && canMove && Time.time >= lastDashTime + dashCooldown)
        {
            StartCoroutine(Dash());
        }
    ...
    */
    ...
    private IEnumerator Dash()
    {
        ...
        // Si el jugador se está moviendo, dashea en esa dirección
        if (_currentDirection != Vector2.zero)
        {
            dashDir = _currentDirection;
        }
        else
        {
            // Si está quieto, dashea hacia donde está el mouse
            Vector2 mousePos = Camera.main.ScreenToWorldPoint(Mouse.current.position.ReadValue());
