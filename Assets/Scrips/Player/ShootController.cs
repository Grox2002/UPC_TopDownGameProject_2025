...
public class ShootController : MonoBehaviour
{
    ...
    public void Shoot()
    {
        // Evita disparar si está en cooldown o sin stamina
        if (Time.time < _nextFireTime || !_playerAttack.ConsumeStamina(_shootStaminaCost))
            return;

        //Asegurar que el arco se rote al mouse ANTES de disparar para evitar el error de la flecha desorientada
        RotateBowTowardsMouse();
