...
public class SeraphBoss : MonoBehaviour
{
    ...
    private IEnumerator PatronAtaque()
    {
        while (_currentHealth > 0)
        {
            ...
            if (newPhase == 3 && !_divinePunishmentActivated)
            {
    ...
    public void TakeDamage(int damage)
    {
        ...
        //  DETECTAR CAMBIOS DE FASE INSTANTÁNEAMENTE
        int newPhase;
        if (_currentHealth > _maxHealth * 0.66f)
            newPhase = 1;
        else if (_currentHealth > _maxHealth * 0.33f)
        ...
        // Si la fase cambió  ejecutar transición al instante
        if (newPhase != _currentPhase)
        {
    ...
    public IEnumerator BossDefeatSequence()
    {
        ...
        // Iniciar diálogo final del boss
        _dialogManager.StartDialogue(_bossDialog);

        // Esperar a que termine el diálogo
        yield return new WaitUntil(() => !_dialogManager.dialogueBox.activeSelf);

        // Hacer que el boss se eleve hacia arriba
        yield return StartCoroutine(BossAscendAnimation());

        // Volver la cámara al jugador
